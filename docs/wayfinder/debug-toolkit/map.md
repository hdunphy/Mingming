...
- Build-out of each surface (launcher UI, battle overlay, save-editor panel, sim CLI) — graduates into implementation tickets as [Debug gating architecture](tickets/03-debug-gating-architecture.md), [Live-manipulation command set](tickets/05-live-manipulation-command-set.md), [Save/run editor verbs](tickets/07-save-run-editor-verbs.md), and [Batch sim & auditor design](tickets/08-batch-sim-auditor-design.md) resolve. The scenario layer's own build-out has already graduated as [Scenario schema & normalizer](tickets/10-scenario-schema-implementation.md) and [Scenario materializer](tickets/11-scenario-materializer.md).
