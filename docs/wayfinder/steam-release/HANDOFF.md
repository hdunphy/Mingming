...
*Last updated: 2026-08-31 (agent sessions: 02, 03, 04, 26, 06, 21, 23, 20, 09-15, 18, 19, 22, 24, 36, 55, 31, 57, 59, 61, 67-build, 67-legion, 68-legion, 42-legion, 70-measure, 71-legion, 72-legion, 73-legion, 73-nerf-arms, 74-legion, 74-close). **State: 75 tickets, 41 closed.** **TICKET 74 CLOSED; TICKET 75 OPEN AND AWAITING HENRY.** Tidewrack fields `jormungandr_v1 + kraken_v2 + skoll_v2`; the boss fight went **30.0% -> 75.0%** and is now the BEST of its three fights (7.9-8.4 turns against 5.8 - TIDAL SURGE slowed, as ruling 1 intended). **`thorn_tithe` is PRINTED at 1 energy / 30 power / 3 Weakened on the TARGET** - Henry's number off the reprice arm, and the shipped printing IS the measured one (75.0%, p = 1.00 paired against 40). `ink_stream` stays at 33, CLOSED. **`experimentalTweaks.ts` now has NO LIVE KNOBS** - every one was ruled on and deleted, and each retired name THROWS naming the ruling that retired it, because a stale command line out of research/73 must not quietly measure the baseline and get filed as a null result. **TICKET 75 IS THE LEFTOVER NUMBER, AND THE FINDING REFRAMES IT.** The gauntlet still fails at **28.1% compound** (favourable, n=60, toolbox) / 38.5% (handbuilt), because `rollGauntletFight` consults `authoredBossFor` for the BOSS SLOT ONLY. But fights 1-2 draw from `regionSpeciesPool` - the union across all three biomes - and `walkOrderFor` steps a 3-cycle over `[Fire, Water, Nature]`, so **every gym's rolled pool is IDENTICAL** (verified: all 12 tuned OS ids at all three gyms, and no Driver). The enemy side of the lead-ins does not differ by gym; **only the counter-party the `favourable` arm brings does** - Nature-leaning reads 61.7/66.7 where Water-leaning read 83.3/90.0. That may be ticket 73's launch triangle surfacing in the gauntlet, in which case 73 and 75 share a lever. **The Emberfall row is NOT comparable** (ticket 68 conditions), so 75's first build step is re-measuring Emberfall and Rootfall at n=60 + toolbox on this tree (~6h) before anything is ruled. Gates green: tsc both configs, eslint 0, 2,094 tests, liveness all LIVE / 0 dark. Full write-up: [research/73-the-tidewrack-nerf-arms.md](research/73-the-tidewrack-nerf-arms.md) SS7.*
