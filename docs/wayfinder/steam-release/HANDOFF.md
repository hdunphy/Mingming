...
*Last updated: 2026-08-31 (agent sessions: 02, 03, 04, 26, 06, 21, 23, 20, 09-15, 18, 19, 22, 24, 36, 55, 31, 57, 59, 61, 67-build, 67-legion, 68-legion, 42-legion, 70-measure, 71-legion, 72-legion, 73-legion, 73-nerf-arms, 74-legion). **State: 74 tickets, 40 closed.** **TICKET 74 BUILT AND MEASURED — WAITING ON HENRY (two questions).** Tidewrack's trio is now `jormungandr_v1 + kraken_v2 + skoll_v2`; `thorn_tithe`'s 3 Weakened are printed on the TARGET. **THE BOSS FIGHT IS FIXED: 30.0% -> 75.0%** against Henry's playtest party, and it is now the BEST of its three fights rather than the worst (fights run 7.9-8.4 turns against 5.8 — TIDAL SURGE firing slower, as ruling 1 intended). **THE GAUNTLET STILL FAILS**, and the failure MOVED: `rollGauntletFight` consults `authoredBossFor` only for the boss slot, so fights 1 and 2 are rolled teams ticket 74 could not touch, and they are now the binding constraint. Compound (67 R5, n=60, toolbox): **favourable 28.1%** (61.7/66.7/68.3) and **handbuilt 38.5%** (70.0/73.3/75.0) against 60+/-5 — FAIL by 31.9 and 21.5pt. Emberfall's calibrated fights are 83.3/90.0/80.0, so Tidewrack's lead-ins sit 13-22pt under. The old 30% boss was MASKING a gauntlet-wide shortfall. **WAITING ON HENRY: (1) is 74 done — its own goal is met, so closing it and opening a ticket on Tidewrack's ROLLED fights is Legion's read but not its call; (2) `thorn_tithe`'s number — the reprice arm says 30 is FREE (75.0%, p = 1.00 paired) and 25 costs ~5pt (70.0%, p = 0.375), nothing printed without his word.** `ink_stream` stays at 33 (74 ruling 2, CLOSED). The `boss-cantrips*` / `ink-power-*` knobs are DELETED and now throw a message naming the ruling. Gates green: tsc (both configs), eslint 0, 2,089 tests, liveness all LIVE / 0 dark. Full write-up: [research/73-the-tidewrack-nerf-arms.md](research/73-the-tidewrack-nerf-arms.md) SS7.*
