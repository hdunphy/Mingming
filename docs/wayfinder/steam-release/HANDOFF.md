...
*Last updated: 2026-09-01 (agent sessions: 02, 03, 04, 26, 06, 21, 23, 20, 09-15, 18, 19, 22, 24, 36, 55, 31, 57, 59, 61, 67-build, 67-legion, 68-legion, 42-legion, 70-measure, 71-legion, 72-legion, 73-legion, 73-nerf-arms, 74-legion, 74-close, 75-table, 76-arms). **State: 76 tickets, 41 closed.** **GRADING CONVENTION (75 ruling 2, NOW STANDING): the BARE arm grades a gym. A toolbox arm is a DIAGNOSTIC line beside it, like the control arm, and a negative toolbox line never fails a gym.** The run-gate banner says which arm it is. Also standing, from 75's own lesson: **a gym number carried across engine or harness changes is STALE until re-taken** - same tree, same day, same arms, or it does not get compared. **TICKET 76 ARMS REPORTED, AWAITING HENRY'S SESSION** - 12 arms, 1,920 battles, [research/76](research/76-rootfall-diagnosed.md). **ROOT ROT is worth +26.6pt on its own**: Rootfall's boss goes 56.7% -> **83.3%** with the Driver stripped, same seeds and trio, 17 fights flipped to the player and 1 against, p = 0.00014 - the largest single-lever effect across 71-76, and TIDAL SURGE was INERT under the same isolation. **But one lever does not close the gym**: a free boss still leaves the compound at ~40.7% against 60, because Rootfall is soft at all three fights (73.3 / 66.7 / 56.7) rather than boss-shaped. The comp candidate `rat_v1 -> rat_v2` is a CLEAN NULL (p = 1.00) - Tidewrack's answer does not transfer. Water-lean beats the type-correct fire-lean party by 10pt of compound while bringing the WRONG element, all of it in the rolled fights (at the boss they tie at 56.7%) - ticket 73's triangle in the gauntlet, p = 0.098, wants n=120. **THE TOOLBOX DIAGNOSIS IS HALF AN ANSWER AND THE HONEST HALF.** Per-card arms at two gyms give OPPOSITE shapes: at Rootfall every card is individually FREE (p ~ 1.00 each) and the basket costs 17pt; at Emberfall each card costs 9-12pt alone and two cost the full 19. So **there is something to reprice at Emberfall and nothing to reprice at Rootfall**, and one ruling covering both would be wrong at one of them. NO MECHANISM IS ASSERTED and none of the three candidates survives: 'bad printings' fails at Rootfall, 'buys too many' fails at Emberfall, energy cost fails too (Emberfall's loss rides its two 1-ENERGY cards). New harness: `--toolbox selective|card:<id>`, `--lean <Element>` (reuses the harness's own picker so a bracket measures the lean, not anyone's team-building), and `--tweak rootfall-rat-v2` as a knob - **the Rootfall trio in the tree is UNCHANGED**. Gates green: tsc both configs, eslint 0, 2,099 tests.*
