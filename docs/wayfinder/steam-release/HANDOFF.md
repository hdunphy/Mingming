...
*Last updated: 2026-08-31 (agent sessions: 02, 03, 04, 26, 06, 21, 23, 20, 09-15, 18, 19, 22, 24, 36, 55, 31, 57, 59, 61, 67-build, 67-legion, 68-legion, 42-legion, 70-measure, 71-legion, 72-legion, 73-legion, 73-nerf-arms, 74-legion, 74-close, 75-table). **State: 75 tickets, 41 closed.** **THE THREE-GYM TABLE HAS BEEN MEASURED PROPERLY FOR THE FIRST TIME, AND TWO THINGS EVERYONE BELIEVED ARE WRONG.** Six arms, 1,080 battles, all three gyms with and without the toolbox at IDENTICAL conditions (n=60, favourable, Rally live, one tree, paired seeds) - [research/75](research/75-the-three-gym-table-and-the-toolbox.md). Compound, no toolbox: **Emberfall 62.4% PASS**, **Tidewrack 40.0%** (-20.0pt), **Rootfall 27.7%** (-32.3pt). With toolbox: 44.0 / 28.0 / 11.1. **(1) TICKET 69'S COUNTER-CARDS ARE A NET NEGATIVE AT EVERY GYM** - they cost the player 11.5 points of win rate, 540 paired battles, p = 0.0000017 (Emberfall p = 0.023, Rootfall p = 0.00019). The cards shipped to make these fights winnable make them harder. Where the damage lands differs by gym (Emberfall: boss only; Rootfall: everywhere; Tidewrack: the rolled fights) and NO mechanism is verified for that pattern. **(2) ROOTFALL IS THE WORST GYM AND HAS NO TICKET** - Tidewrack got three (71/73/74) while being the MIDDLE gym. Its last number (67 R5, '~7.6pt under') predates the toolbox, the firmware-pairing fix, the draw-scoping fix and the biome walk-order inversion, and was carried forward across all four. **Emberfall's calibration HOLDS** at 62.4% bare. **WAITING ON HENRY (ticket 75, restated): Q1 what happens to the toolbox** (pull / reprice / keep / drop it from the graded arm - it contradicts both ticket 69's design and 74's 'toolbox purchasable'); **Q2 which arm grades a gym** (every historical number is toolbox-OFF, every number reported in the last two days is toolbox-ON - pick one or this recurs); **Q3 does Rootfall get its own build ticket.** Ticket 74 stays CLOSED and its own result stands (boss 30.0% -> 75.0%); `thorn_tithe` is printed at 1e / 30 power / 3 Weakened on the TARGET, and the re-run confirms the reprice was neutral (28.0% vs 28.1%). Gates green: tsc both configs, eslint 0, 2,094 tests.*
