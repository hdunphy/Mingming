...
## Rulings round 2 (Henry, 2026-08-27) — closes the open items in research/67-gate-validity-and-the-power-ceiling.md §9

1. **Control arm CONFIRMED as Legion recommended:** same-element team at party sizes 1–2; at size 3, two same-element + one third member whose matchup is RECORDED beside the number; the arbitrary-lineup population mean stays as the free third line. Control and prepared arms run identical shipping combat code — the flattened-matrix variant is rejected.
2. **"Average prepared" PINNED as the full reading:** counter-element where roster + blueprints allow, a shopped/pruned deck (harness gains a simple prepared-deck builder: generics sold, ~2 on-pool picks per biome, anti-boss cards in by the gauntlet), holding the Q2 anti-boss cards at the gym — not a solved line. **95/75/60 grades this arm**; the control arm is diagnostic.
3. **Anti-boss tier acquisition RULED:** the 2–3 cards per deck stay INSIDE the party's species pools; they appear as **elite rewards + a premium marketplace slot** (a visible chase before the gym); **the enemy never gets them** — the one deliberate player-pointing asymmetry, stated here so nobody "fixes" it; the 12 existing off-deck launch-element Rares are candidates, the design session decides per-deck. **The card design itself is a deck-archetypes request** (24–36 cards; a Henry design session in the mingming-deck-pass shape), not a Legion ticket. §9's boss-firmware lever table is the design brief's seed.
4. **ICE_RELIC vs jormungandr_v2 is INTENDED counter-play** — types are visible at run start; routing around the counter gym is the routing layer working. Constraint for the design session: jorm_v2's anti-boss cards must give it SOME non-Poison line into the Ice gym — disfavored, never deleted.

**Sequencing:** (1) run the §8 boss diagnostic NOW (counter-element team vs fixed boss, ~40 min — pre-check on whether the boss is a wall before the cards exist); (2) Legion builds the control + prepared gate arms (prepared-deck builder included) — buildable before the cards land, graded only after; (3) the anti-boss design session with Henry; (4) full six-number re-measure; (5) grade vs 95/75/60, then the numbers grilling if bands still miss; (6) playtest round 6.
