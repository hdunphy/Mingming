...
*Last updated: 2026-08-06, after ticket 21 landed. If you are a fresh session (any model): read this, then map.md, then the ticket you're assigned.*

## Where things stand

- Branch **card-dev**. Tickets 01–21 closed.
- **Water complete** (ticket 20: kraken §2.3 43/57, jorm 51/49, mirrors clean).
- **Sleipnir complete** (ticket 21: §2.3 0.00 → **0.59**, mirror 400/400 decided at 2.5 turns, ftk 0, no knob rounds spent). The lesson worth carrying: the gap was the OS, not the cards — MOMENTUM_DRIVE paid 1 Strengthened per 0-cost card into a ±25% status cap, so v1's whole engine was invisible. Fixed by 1→2 stacks plus a `STRENGTH_STACKS` power scaler that reads RAW stacks, and a payoff card (Momentum Crash).
...
- **Hraesvelgr is the other half of Air** — ticket 22, in flight in the same session as 21. 11 species remain after it.
...
1. Finish Air: ticket 22 (hraesvelgr) — v1 GALE_FORCE discard windmill, v2 UPDRAFT burn-X ramp, X-cost mechanic, `gust_jab` retired into Tackle.
