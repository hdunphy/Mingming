...
**Measured:** full run 3m36s on this 2-core sandbox — CPU-saturated, so sharding can't show its gain here; on an 8-core machine the critical path becomes the longest single shard (≈1500–1600 battles ≈ 60–90s), which is the ticket's target. **Scoped run: 10.0s, measured.** The next deck pass's tuning loop costs seconds per iteration with the full committed run reserved for the final gate — record Henry's real-machine full-run time here when he next runs it.
