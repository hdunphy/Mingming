...
2. **1-turn lookahead**: let the search evaluate sequences one turn past END_TURN — statuses tick, energy refills (Energized cashes), then value the best own reply next turn. Ramp gets judged by what it enables instead of a constant. Budget: keep total sim cost within ~2–4× (ticket 17's sharding + scoped mode absorb this); prune if needed.
