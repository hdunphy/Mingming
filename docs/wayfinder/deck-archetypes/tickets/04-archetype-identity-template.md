...
- **Differentiation bar:** the two decks of one species share an element and (mostly) a card pool — what must differ for them to *feel* like two archetypes? NorseExpansion's shapes (burn-stacking vs high-risk burst, control vs energy-denial) are the inspiration reference, filtered through the audit's keep/adapt/reject list.
